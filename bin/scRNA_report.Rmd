...
exon_ratio <- fromJSON(params$featureStats)$exonRatio
intron_ratio <- fromJSON(params$featureStats)$exonRatio
intergenic_ratio <- fromJSON(params$featureStats)$intergenicRatio
```
...
d <- starsolo_summary %>% 
    ...
    add_row(item = "Reads Mapped to Intergenic Regions", value = intergenic_ratio)
names(d) <- c("","")
d %>% kbl() %>% kable_paper("hover", html_font = "arial, helvetica, sans-serif")
```
...
```{r}
dep <- read_tsv("scaled.tab", skip=3, col_names = NULL) %>% apply(2, sum, na.rm = TRUE)
...
ggplot(geneCoverage_df, aes(x=`pos`, y=`dep`)) +
    ...
    xlab("Relative Position")+ylab("Transcript Coverage")+
