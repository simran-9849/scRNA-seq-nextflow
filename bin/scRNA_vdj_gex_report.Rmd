...
## defined as the same VDJ segements and > 80% CDR3 nt identity
getLineage <- function(cells, barcode_report){
    ...
    d$chain1_V <- strsplit(d$chain1,",") %>% sapply("[",1)
    d$chain1_D <- strsplit(d$chain1,",") %>% sapply("[",2)
    d$chain1_J <- strsplit(d$chain1,",") %>% sapply("[",3)
    d$chain1_cdr3 <- strsplit(d$chain1,",") %>% sapply("[",5)
    
    d$chain2_V <- strsplit(d$chain2,",") %>% sapply("[",1)
    d$chain2_D <- strsplit(d$chain2,",") %>% sapply("[",2)
    d$chain2_J <- strsplit(d$chain2,",") %>% sapply("[",3)
    ...
    d_multiple %>% lapply(function(x){
        k <- x %>% pull(chain1_cdr3)
        ...
        thre <- 1 + (length(k)-1)*0.8
        if(any(colSums(identityM) < thre)){
            m <- x[colSums(identityM) < thre, ]
            for(i in 1:nrow(m))
            return(x[colSums(identityM) >= thre, ])
        }else{
            return(x)
        }
    })
...
## Rename UMI terms
if(!withUMI){
    vdj_t_tbl$term[which(vdj_t_tbl$term=="Mean UMIs per Cell")] <- "Mean Reads per Cell"
    vdj_t_tbl$term[which(vdj_t_tbl$term=="Median UMIs per Cell")] <- "Median Reads per Cell"
}
...
## Rename UMI terms
if(!withUMI){
    vdj_b_tbl$term[which(vdj_b_tbl$term=="Mean UMIs per Cell")] <- "Mean Reads per Cell"
    vdj_b_tbl$term[which(vdj_b_tbl$term=="Median UMIs per Cell")] <- "Median Reads per Cell"
}
