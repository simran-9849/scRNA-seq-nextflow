...
expl_tbl <- tribble(
    ~title, ~content,
    "GEX Association",
    "Detected BCR CDR3 clones were projected to cells' UMAP-plot, only paired productive clones were shown."
)
...
expl_tbl <- tribble(
    ~title, ~content,
    "Knee Plot",
    'Knee Plot was also called as Barcode Rank Plot. The plot shows total UMI (y axis) of each associated barcode (x axis). Foreground cells were selected via cell-calling per "Knee point" as typical scRNAseq experiment, Target (e.g. B/T) cells were further deterimined by their target mRNA UMI (e.g. VDJ UMI) counts (>3). For B or T cells, we also require their expressed BCR/TCRs to be productive.'
)
...
p1 <- ggplot(pie_d, aes(x = "", y = count, fill = labels))+geom_col(color = "black") +
    geom_text(aes(label = percentage),
                  ...
                  color = "white") +
    coord_polar(theta = "y")+
    scale_fill_manual(values = c("#4169E1","grey"))+
    theme_void()+
