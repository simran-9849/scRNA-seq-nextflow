...
## defined as the same VDJ segments, identical CDR3 length and > 80% CDR3 nt identity
...
expl_tbl <- tribble(
    ~title, ~content,
    "Clones (Paired)",
    "Here we counted unique CDR3 amino acid pairs with identical germline VDJ gene segments as different clones. This result includes all the paired ones from the cloneType table.",
    "Lineage Number",
...
expl_tbl <- tribble(
    ~title, ~content,
    "Top 1000 Clonetypes",
    "Clonetypes are defined as the same VH and VL CDR3 amino acid sequences as well as identical VDJ combinations. Here we also require the chains to be productive, and cells with only one chain detected are also included."
)
...
expl_tbl <- tribble(
    ~title, ~content,
    "Top 10 Clonetypes",
    "The histogram depicts cell counts of the top 10 abundant clonetypes generated by <b>productive</b> chains. Clonetypes with only one chain detect are also included. The clonotype IDs on the X axis correspond to the clonotype IDs listed in the table."
)
...
expl_tbl <- tribble(
    ...
    "Total Clone Types Detected",
    "Number of clone types detected. Clonetypes are defined as the same VH and VL CDR3 amino acid sequences as well as identical VDJ combinations."
)
...
d <- combineInfo(cells = vdj_t_cells$CB, 
...
vdj_t_cloneType <- d %>%
    ...
    ungroup() %>%
    mutate(
        label1 = str_sub(chain1_V, 1L, 3L),
        label2 = str_sub(chain2_V, 1L, 3L),
    ...
    mutate(Frequency = cellCount/nrow(d)) %>%
    dplyr::select(cloneType, Hchain_VDJ, Lchain_VDJ, cellCount, Frequency)

vdj_t_cloneType$Hchain_VDJ[which(vdj_t_cloneType$Hchain_VDJ == "*|NA|NA")] <- NA
vdj_t_cloneType$Lchain_VDJ[which(vdj_t_cloneType$Lchain_VDJ == "*|NA")] <- NA
...
### Top 10 Clonetypes `r infoIcon("Clonetypes_top10")`
...
write_tsv(tcr_lineageInfo, file = paste0(params$sampleName, "_TCR_resOut.tsv"))
...
vdj_t_metrics_lineage <- vdj_t_metrics %>% 
    as.data.frame %>% t() %>% as.data.frame() %>% 
    rownames_to_column("term") %>%
    dplyr::rename("value" = "V1") %>%
    bind_rows(lineage_tbl)

write_tsv(vdj_t_metrics_lineage,
          paste0(params$sampleName, "_VDJ-T.metrics.tsv"), 
          col_names=FALSE)

vdj_t_metrics_lineage %>%
    split(vdj_t_metrics_lineage$term) %>% 
...
d <- combineInfo(cells = vdj_b_cells$CB, 
...
vdj_b_cloneType$Hchain_VDJ[which(vdj_b_cloneType$Hchain_VDJ == "*|NA|NA")] <- NA
vdj_b_cloneType$Lchain_VDJ[which(vdj_b_cloneType$Lchain_VDJ == "*|NA")] <- NA
...
write_tsv(bcr_lineageInfo, file = paste0(params$sampleName, "_BCR_resOut.tsv"))
...
vdj_b_metrics_lineage <- vdj_b_metrics %>% 
    as.data.frame %>% t() %>% as.data.frame() %>% 
    rownames_to_column("term") %>%
    dplyr::rename("value" = "V1") %>%
    bind_rows(lineage_tbl)

write_tsv(vdj_b_metrics_lineage,
          paste0(params$sampleName, "_VDJ-B.metrics.tsv"), 
          col_names=FALSE)

vdj_b_metrics_lineage %>%
    split(vdj_b_metrics_lineage$term) %>% 
    lapply(FUN=function(x) { x$value }) %>% 
