...
vdj_t_knee <- read_tsv(params$VDJ_T_kneeData, col_names = c("CB", "UMI"))
vdj_t_knee <- vdj_t_knee %>% 
    mutate(
        cell = c(1:nrow(vdj_t_knee)),
        group = case_when(
            cell <= cellNum ~ "cell",
            TRUE ~ "background"
            )
...
vdj_t_tbl <- tribble(
  ~term, ~value,
  "Reads Mapped to Genome: Unique+Multiple", vdj_t_metrics$totalMappedReads,
  "Reads Mapped to Genome: Unique", vdj_t_metrics$uniquelyMappedReads##,
  ##"Reads Mapped to Any V(D)J Gene", "2",
  ##"Reads Mapped to TRB", 2,
  ##"Reads Mapped to TRD", 2,
  ##"Reads Mapped to TRA", 2,
  ##"Reads Mapped to TRG", 2
) %>%
  mutate(value = as.numeric(value)) %>%
  mutate(value=case_when(value <= 1 ~ label_percent(0.01)(value),
                         value > 1 ~ label_comma()(round(value))))
...
### V(D)J Annotation & T Cell Expression
...
vdj_b_knee <- read_tsv(params$VDJ_B_kneeData, col_names = c("CB", "UMI"))
vdj_b_knee <- vdj_b_knee %>% 
    mutate(
        cell = c(1:nrow(vdj_b_knee)),
        group = case_when(
            cell <= cellNum ~ "cell",
            TRUE ~ "background"
            )
...
vdj_b_tbl <- tribble(
  ~term, ~value,
  "Reads Mapped to Genome: Unique+Multiple", vdj_b_metrics$totalMappedReads,
  "Reads Mapped to Genome: Unique", vdj_b_metrics$uniquelyMappedReads##,
  ##"Reads Mapped to Any V(D)J Gene", 0,
  ##"Reads Mapped to IGH", 0,
  ##"Reads Mapped to IGK", 0,
  ##"Reads Mapped to IGL", 0
) %>%
  mutate(value = as.numeric(value)) %>%
  mutate(value=case_when(value <= 1 ~ label_percent(0.01)(value),
                         value > 1 ~ label_comma()(round(value))))
