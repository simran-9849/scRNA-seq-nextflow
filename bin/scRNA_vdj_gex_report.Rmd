...
expl_tbl <- tribble(
    ~title, ~content,
    "GEX Association",
    "Detected BCR CDR3 clones were projected to cells' UMAP-plot."
)
...
p1 <- ggplot(pie_d, aes(x = "", y = count, fill = labels))+geom_col(color = "black") +
    geom_text(aes(label = percentage),
                  ...
                  color = "white") +
    coord_polar(theta = "y")+
    scale_fill_manual(values = c("#3f48CC","grey"))+
    theme_void()+
    theme(legend.title = element_blank(), legend.position = "top", legend.direction = "vertical")

p2 <- ggplot(barplot_d, aes(x=rank, y = cellNum))+geom_bar(stat = "identity", fill = "#3f48CC", color = "black")+
